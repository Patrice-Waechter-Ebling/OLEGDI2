...
__declspec(dllexport) char* GetDLLVersion(){return Version;};
...
__declspec(dllexport) char* Int2Str(int Valeur){char dbl[25];wsprintf(dbl,"%d",Valeur);	return dbl;}
__declspec(dllexport) char* Double2Str(int Valeur){ char dbl[25];wsprintf(dbl,"%f",Valeur);	return dbl;}
__declspec(dllexport) int Str2Int(char* Valeur){    int x;     sscanf(Valeur, "%d", &x); 	return x;}
...
__declspec(dllexport) char* ReadDatas(char* Section, char* Clef, char* Source){	char tmp[260];	GetPrivateProfileString(Section,Clef,"?",tmp,260,Source);	return 	tmp;}
...
__declspec(dllexport) int SetDefautVersion(char* Source){	return UpdateDatas("DATAS","Version","3",Source);}
__declspec(dllexport) int UpdateDatas(char* Section, char* Clef, char* Valeur, char* Cible){	return WritePrivateProfileString(Section,Clef,Valeur,Cible)	;}
__declspec(dllexport) int UpdateDatas(char* Section, char* Clef, double Valeur, char* Cible){	char tmp[80]; wsprintf(tmp,"%f",Valeur);return WritePrivateProfileString(Section,Clef,tmp,Cible)	;}
...
__declspec(dllexport) int GetApproximateStep(int width){return  (int)(width/(m_nbBits+20));}
__declspec(dllexport) void errhandler(LPCTSTR Texte,HWND hwnd){MsgBox(hInst,hwnd,(char*)Texte,"Gestionnaire d'erreur",0,100);}
...
__declspec(dllexport) int GetAppLastError(){    return m_iError;}
__declspec(dllexport) const char* GetCodeString(){    return m_szData;}
...
__declspec(dllexport) int GetApproximateWidth(int step){    if(!m_nbBits) return 0;     return (m_nbBits+20)*step;}
__declspec(dllexport) int CheckDigit(const char * pszCode, BOOL bComplet){    return 0;}
